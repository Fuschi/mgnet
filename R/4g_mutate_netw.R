...
#'        metadata columns to distinguish the nodes connected from an edge. It is used from `gather_link`.
...
setGeneric("mutate_netw", function(object, ..., group_taxa, .suffix = c("_1", "_2")) {standardGeneric("mutate_netw")})

setMethod("mutate_netw", "mgnet", function(object, ..., group_taxa, .suffix = c("_1", "_2")) {
  ...
  for(i in seq_along(quosures)){
    ...
    if(missing(group_taxa)){
          modified_text <- str_replace_all(text, "netw", "netw(object, add_vertex_attr = TRUE)")
          modified_text <- str_replace_all(modified_text, "comm", "comm(object)")
    } else {
          modified_text <- str_replace_all(text, "netw", "netw(object_subset, add_vertex_attr = TRUE)")
...
setMethod("mutate_netw", "mgnetList", function(object, ..., group_taxa, .suffix = c("_1", "_2")) {
  ...
  for(i in seq_along(quosures)){
    ...
    if(missing(group_taxa)){
      modified_text <- str_replace_all(text, "netw", "netw(object[[mg]], add_vertex_attr = TRUE)")
      modified_text <- str_replace_all(modified_text, "comm", "comm(object[[mg]])")
    } else {
      modified_text <- str_replace_all(text, "netw", "netw(object_subset[[mg]], add_vertex_attr = TRUE)")
  ...
  if (missing(group_taxa)) {
    ...
    subgroups <- gather_taxa(object) %>%
      dplyr::select(tidyselect::all_of(group_taxa)) %>%
      dplyr::unite("_internal_", remove = FALSE) %>%
      split[.[["mgnet"]]]

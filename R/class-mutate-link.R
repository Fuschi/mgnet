...
#'
#' If link grouping is active, expressions are evaluated within link groups.
#'
...
#' @importFrom dplyr mutate group_by ungroup left_join select
#' @importFrom rlang .data
#' @importFrom tidyselect any_of
#' @export
setGeneric("mutate_link", function(object, ..., .ungroup = FALSE, .deselect = FALSE) {
  standardGeneric("mutate_link")
})
...
setMethod("mutate_link", "mgnet", function(object, ..., .ungroup = FALSE, .deselect = FALSE) {
  ...
  link(object) <- dplyr::mutate(edges, !!!quos) %>%
    dplyr::ungroup() %>%
